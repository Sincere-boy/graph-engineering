...
async def test_dispatcher_notifies_user_in_group_when_workflow_completes(
    ...
    botmux = FakeBotmux(
        '{"delivery_id":"completion-d1","message_id":"om_completion"}'
    )
    ...
    assert "面向用户发送完成通知" in call["instruction"]
    assert "botmux send --no-mention" in call["instruction"]
    assert "botmux dispatch" not in call["instruction"]
    assert "不得只在最终回答中返回摘要" in call["instruction"]
